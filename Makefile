...
train:
	$(PYTHON) -m pipeline.train --manifest volunteer_portal/data/records.jsonl --output runs/latest
...
check:
	$(PYTHON) -m py_compile volunteer_portal/app.py volunteer_portal/storage.py pipeline/dataset.py pipeline/model.py pipeline/train.py scripts/pull_and_build_components.py scripts/check_wan_latest.py scripts/configure_and_push_remote.py
